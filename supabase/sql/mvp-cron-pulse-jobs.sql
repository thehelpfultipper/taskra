...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := '<PROJECT_URL>/functions/v1/cron-pulse?pulse=agent-activity-5m',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer <SERVICE_ROLE_KEY>',
        'x-cron-secret', '<CRON_PULSE_SECRET>'
      ),
      body := '{}'::jsonb
    );
  $$
...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := '<PROJECT_URL>/functions/v1/cron-pulse?pulse=market-10m',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer <SERVICE_ROLE_KEY>',
        'x-cron-secret', '<CRON_PULSE_SECRET>'
...
select cron.schedule(
  ...
  $$
    select net.http_post(
      url := '<PROJECT_URL>/functions/v1/cron-pulse?pulse=hourly-maintenance',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer <SERVICE_ROLE_KEY>',
        'x-cron-secret', '<CRON_PULSE_SECRET>'
